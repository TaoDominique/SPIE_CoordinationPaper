...
\begin{document} 
	...
	As dictated by the principle of the potential field, each positioner is drawn to its target by an attractive force $F_a$ and avoiding its $i^{ieme}$ neighouring positioners with repulsive forces, $F_{ri}$. Further details on the forces implementations are given in [\citenum{MakaremMoons}]. \\
...
\label{key}			}
	...
	\end{itemize}\\
	 ...
	 If both positioners are OFF (line \ref{condition1}), then it means that both have retained the information on the higher priority positioners which put them OFF. By comparing their "MemPr" states and in case the agent positioner one is smaller, it will have its "MemPr" states changed equalling the one from its neighbour. One can view it as having the agent positioner being influenced, through the neigbour positioner, by the high priority positioner that put this neighbour positioner OFF, such that the agent positioner will not oppose the movement of this high priority positioner towards its target.\\
	 The trivial case (line \ref{condition2}) is when two positioners are ON. The comparison is then just with their pre-assigned priorities to determine if the agent positioner will be OFF and memorize the Id and pre-assigned priority information from the other positioner.\\
	 The two last mixed cases is when the agent and the neighbour positioner have different ON/OFF states (line \ref{condition3}, \ref{condition4}).  When the agent positioner is ON and have a pre-assigned priority smaller than the memorized priority "MemPr" of the neighbour positioner, the agent is becoming OFF with having the same memorized priority than the OFF neighbours positioner for the same reason as explained with the case of both positioners being OFF. \\
	 In the other case when the agent positioner is OFF with its neighbour positioner being ON, the priority comparison will be done between the one that the agent has memorized when it became OFF with the pre-assigned priority of the neighbour. if smaller, it will change its "MemPr" to remember the higher priority one. \\\\
	...
	\begin{algorithm}[H]
		...
		{		\BlankLine
			\uIf{agent == OFF and neighbour == OFF \label{condition1}}{
						\BlankLine
				\uIf{agent.MemPr $<$ neighbour.MemPr}{
					\BlankLine
			...
			\uElseIf{agent == ON and neighbour == ON \label{condition2}}{
						\BlankLine
				\uIf{Agent Priority $<$ Neighbour Priority}{
					\BlankLine
					agent.ON/OFF = OFF \;\\
					agent.MemId =  Neighbour Id\;\\
			...
			\uElseIf{agent == OFF and neighbour == ON \label{condition3}}{
						\BlankLine
				\uIf{agent.MemPr $<$ Neighbour Priority}{
					\BlankLine
					agent.MemId =  Neighbour Id\;\\
					agent.MemPr =  Neighbour Priority\;\\
				}
			...
			\uElseIf{agent == ON and neighbour == OFF \label{condition4}}{
						\BlankLine
				\uIf{Agent Priority $<$ neighbour.MemPr}{
					\BlankLine
					 agent.ON/OFF = OFF \;\\
					agent.MemId =  Neighbour Id\;\\
					agent.MemPr =  Neighbour.MemPr\;
